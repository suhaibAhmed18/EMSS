...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Check if we just connected a store
    ...
    if (connected && status === 'success') {
      ...
      // Refetch dashboard data to show the new store
      setTimeout(() => {
        fetchDashboardData()
      }, 1000)
    }
  ...
  const syncShopifyData = async () => {
    try {
      ...
      toast({
        type: 'info',
        title: 'Syncing...',
        description: 'Fetching latest data from your Shopify store',
        duration: 3000
      })
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.details || 'Failed to sync store data')
      }
      ...
      toast({
        type: 'success',
        title: 'Sync Complete!',
        description: `Synced ${result.metrics.ordersSynced} orders and ${result.metrics.customersImported + result.metrics.customersUpdated} customers`,
        duration: 5000
      })
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button onClick={fetchDashboardData} className="btn-ghost" disabled={loading || syncing}>
