...
export class WorkflowActionExecutor {
  ...
  private async executeSendEmailAction(
    ...
    // Get workflow settings from trigger data
    const workflowSettings = (context.triggerData as any).workflowSettings || {}
    const sendToSubscribedOnly = workflowSettings.send_to_subscribed_only !== false // Default to true
    const respectQuietHours = workflowSettings.respect_quiet_hours || false

    // Check email consent if send_to_subscribed_only is enabled
    if (sendToSubscribedOnly && !contact.email_consent) {
      throw new Error('Contact has not consented to email marketing (send_to_subscribed_only is enabled)')
    }

    // Check quiet hours if respect_quiet_hours is enabled
    if (respectQuietHours && this.isQuietHours()) {
      throw new Error('Cannot send email during quiet hours (respect_quiet_hours is enabled)')
    }
  ...
  private async executeSendSMSAction(
    ...
    // Check SMS consent if send_to_subscribed_only is enabled
    if (sendToSubscribedOnly && !contact.sms_consent) {
      throw new Error('Contact has not consented to SMS marketing (send_to_subscribed_only is enabled)')
    }

    // Check quiet hours if respect_quiet_hours is enabled
    if (respectQuietHours && this.isQuietHours()) {
      throw new Error('Cannot send SMS during quiet hours (respect_quiet_hours is enabled)')
    }
