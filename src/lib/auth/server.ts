...
class AuthServer {
  ...
  async signUp(email: string, password: string, name?: string): Promise<{ user: User, needsVerification: boolean }> {
    const supabase = getSupabaseAdmin()
    ...
    try {
      ...
      const { data: user, error } = await supabase
        ...
        .insert({
          ...
          email_verified: true // Disable email verification - auto-verify all users
        })
        .select()
        .single()
