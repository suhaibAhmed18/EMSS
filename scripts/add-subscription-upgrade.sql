...
-- ============================================================================
-- This script adds upgrade functionality with immediate payment
-- Users can upgrade anytime and pay the difference
-- ============================================================================
...
-- ============================================================================
-- Calculate how much user needs to pay when upgrading mid-cycle
-- ============================================================================
...
BEGIN
  ...
  -- Calculate upgrade cost (new plan price - prorated credit)
  -- For immediate upgrade, charge full new plan price minus credit
...
-- ============================================================================
-- Call this after successful upgrade payment
-- ============================================================================
...
-- ============================================================================
-- Schedule a downgrade to happen at next billing cycle (no immediate payment)
-- ============================================================================
...
-- ============================================================================
-- Run this at billing time to process scheduled downgrades
-- ============================================================================
...
DECLARE
  ...
  v_user RECORD;
BEGIN
  -- Process all users with scheduled downgrades whose subscription expired
  FOR v_user IN 
...
-- ============================================================================
-- Get all plans available for upgrade (higher price than current)
-- ============================================================================
...
-- Example 1: Calculate upgrade cost
-- SELECT * FROM calculate_upgrade_cost('user-uuid', 'enterprise');

-- Example 2: Get available upgrades for user
-- SELECT * FROM get_available_upgrades('user-uuid');

-- Example 3: Upgrade subscription (after payment)
-- SELECT * FROM upgrade_subscription('user-uuid', 'enterprise', 'payment-id-123');

-- Example 4: Schedule downgrade (no immediate payment)
-- SELECT * FROM schedule_downgrade('user-uuid', 'starter');

-- Example 5: Cancel scheduled downgrade
-- SELECT * FROM cancel_scheduled_downgrade('user-uuid');

-- Example 6: Process scheduled downgrades (run at billing time)
-- SELECT * FROM process_scheduled_downgrades();

-- Example 7: View upgrade history
...
DO $$
BEGIN
  RAISE NOTICE '✅ Subscription upgrade system setup completed!';
  RAISE NOTICE '📈 Functions created:';
  RAISE NOTICE '   - calculate_upgrade_cost() - Calculate prorated upgrade cost';
  RAISE NOTICE '   - upgrade_subscription() - Process immediate upgrade';
  RAISE NOTICE '   - schedule_downgrade() - Schedule downgrade for next billing';
  RAISE NOTICE '   - get_available_upgrades() - Get all available plans';
  RAISE NOTICE '   - process_scheduled_downgrades() - Process scheduled changes';
  RAISE NOTICE '';
  RAISE NOTICE '💡 Upgrade Flow:';
  RAISE NOTICE '   1. User clicks upgrade → calculate_upgrade_cost()';
  RAISE NOTICE '   2. User pays → upgrade_subscription()';
  RAISE NOTICE '   3. Subscription extended by 1 month with new plan';
END $$;
